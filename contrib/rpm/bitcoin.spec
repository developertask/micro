...
Source10:	https://raw.githubusercontent.com/MBC/MBC/master/contrib/debian/examples/bitcoingold.conf
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 3132
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 3133
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 13132
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 13133
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 3132
	%{_sbindir}/semanage port -d -p tcp 3133
	%{_sbindir}/semanage port -d -p tcp 13132
	%{_sbindir}/semanage port -d -p tcp 13133
	for selinuxvariant in %{selinux_variants}; do
